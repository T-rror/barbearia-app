// src/components/admin/HistoricoResumo.jsx

export default function HistoricoResumo({ historico }) {
  const totalCortes = Object.values(historico).reduce((acc, qtd) => acc + qtd, 0)
  const hoje = new Date()
  const doisMesesAtras = new Date()
  doisMesesAtras.setMonth(hoje.getMonth() - 2)
  const periodo = `${doisMesesAtras.toLocaleDateString()} até ${hoje.toLocaleDateString()}`
  ...
  return (
    <footer className="mt-16 border-t pt-6 text-center text-sm text-gray-600">
      ...
      <p>Total de cortes realizados: <strong>{totalCortes}</strong></p>
